...
class App(customtkinter.CTk):
    ...
    def btn_mostrar_on_click(self):
        ...
        """ 
        menor = edad_int < 18
        no_soltero = estado_civil != "Soltero"
        mayor_y_soltero = not menor and not no_soltero

        if mayor_y_soltero:
            alert(title = "Respuesta", message = f"Su edad es {edad} y su estado civil es: {estado_civil}. Es soltero y no es menor.")
        """
        #se puede resolver así de forma más concisa:
